...
- [ ] Server concern boundaries are expressed at the file level, but not at the package level — `cmd/server/game_mechanics.go:1`, `cmd/server/connection.go:1`, `cmd/server/dashboard.go:1`, `cmd/server/metrics.go:1`, `cmd/server/error_recovery.go:10` — The code is commented and grouped by concern, yet mechanics, transport, monitoring, and recovery still compile into one package. That keeps internal calls easy, but it broadens ownership and makes routine changes span a large `main` package instead of isolated layers. This is maintainability friction rather than a runtime defect. — **Remediation:** Split server responsibilities into focused packages after the engine extraction: for example `serverengine` for rules/state, `transport/ws` for WebSocket/HTTP handlers, `monitoring` for metrics/dashboard/health, and `validation` for recovery logic. Keep `cmd/server` responsible only for wiring these packages together. Validate with `go build ./...`, `go test -race ./...`, and `/home/user/go/bin/go-stats-generator analyze . --sections packages,functions,structs`.
