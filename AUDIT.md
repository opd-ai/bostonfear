...
- [x] Go client and server packages manually mirror wire/protocol structs instead of sharing a single owner for JSON contracts — `serverengine/game_types.go`, `client/ebiten/state.go`, `client/ebiten/net.go`, `protocol/protocol.go` — The shared Go WebSocket schema now lives in `protocol`, and both `serverengine` and the Ebitengine client compile against those shared enums and payload structs via type aliases instead of maintaining separate copies. This removes the prior Go-to-Go schema drift risk while keeping UI-only client state and connection-quality handling local. — **Validation:** `go test ./protocol ./client/ebiten ./serverengine`, `go vet ./...`, and `xvfb-run -a go test -race ./...`.
