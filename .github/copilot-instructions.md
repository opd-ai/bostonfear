...
6. Respect project security and deployment constraints: keep the repository rules-only and copyright-safe, preserve origin validation through `network.allowed-origins`, and prefer structured logging through `serverengine/common/logging` rather than ad hoc `log.Printf`-style output.
