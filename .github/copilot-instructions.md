# Project Overview
BostonFear is a rules-only, copyright-safe Arkham Horror engine for cooperative multiplayer play. The project centers on a Go server that owns game rules, turn progression, dice resolution, resources, doom escalation, and reconnection state, plus a Go/Ebitengine client that renders the board and UI for desktop, web (WASM), and mobile from one codebase. The default fully implemented module is `arkhamhorror`, while `eldersign`, `eldritchhorror`, and `finalhour` remain scaffolded modules selected through runtime configuration.
...
5. Treat the Ebitengine client as a multi-scene, cross-platform Go application. Reuse shared client state and networking code, preserve 800x600 logical layout assumptions, and avoid introducing platform-specific forks when the same behavior can stay in shared Go code.
